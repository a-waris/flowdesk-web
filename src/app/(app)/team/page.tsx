...
import { UserPlus, Mail, MoreHorizontal, Trash2, Clock, Shield } from 'lucide-react';
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-sm font-medium text-muted-foreground flex items-center gap-2">
            <Shield size={14} /> Members · {members.length}
          </CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            <div className="space-y-3 px-5 pb-5">
              {[0, 1, 2].map(i => <div key={i} className="h-12 rounded-lg bg-muted/40 animate-pulse" />)}
            </div>
          ) : members.length === 0 ? (
            <p className="px-5 pb-5 text-sm text-muted-foreground">No members yet.</p>
          ) : (
            <div className="divide-y divide-border">
              {members.map(m => (
                <div key={m.user_id} className="flex items-center justify-between px-5 py-3 hover:bg-muted/20 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">{m.user_email}</p>
                    </div>
                  </div>
            ...
            <div className="divide-y divide-border">
              {invitations.map(inv => (
                <div key={inv.id} className="flex items-center justify-between px-5 py-3 hover:bg-muted/20 transition-colors">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="text-[10px] text-warning border-warning/20 bg-warning/10">
                      pending
                    </Badge>
                    <Button
                      variant="ghost"
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3 py-2">
            <p className="text-sm text-muted-foreground">
              They&apos;ll receive an email to sign up and join your workspace.
            </p>
            <Input
              type="email"
